...
RDEPENDS_${PN} += "${@bb.utils.contains('DISTRO_FEATURES', 'pam', 'pam-plugin-keyinit pam-plugin-limits', '', d)}"
...
CONFFILES_${PN}-lib = "${sysconfdir}/sudoers"

SUDO_PACKAGES = "${PN}-lib ${PN}-sudo"

PACKAGE_BEFORE_PN = "${SUDO_PACKAGES}"

RDEPENDS_${PN}-sudo = "${PN}-lib"
RDEPENDS_${PN}_append = " ${SUDO_PACKAGES}"
