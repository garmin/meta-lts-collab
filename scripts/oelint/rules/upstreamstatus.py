...
class LTSCollabRulePatchUpstreamStatus(Rule):
    def __init__(self) -> None:
        super().__init__(
            id="oelint.ltscollab.upstreamstatus",
            severity="warning",
    ...
    def check(self, _file: str, stash: Stash) -> list[tuple[str, int, str]]:
        ...
        for i in patches:
            with open(i) as input_:
                ...
                try:
                    cnt = input_.read()
                    for m in RegexRpl.finditer(
                        r"^Upstream-Status:\s*(?P<class>.*)",
                        cnt,
                        flags=regex.regex.MULTILINE,
                    ):
                        found = True
                        if not any(
                            RegexRpl.match(v, m.group("class"))
                            for k, v in valid_class.items()
                        ):
                            _msg = "Upstream-Status in '{FILE}' doesn't pick from valid classifiers {cls}".format(
                                FILE=os.path.basename(i),
                                cls=",".join(sorted(valid_class.keys())),
                            )
                            res += self.finding(
                                recipe_match.Origin,
